...
export default function Card({ name, position, revealed, index }) {
  ...
  return (
    <div
      className={clsx(
        "w-[7.6vw] aspect-[2/3] [prespective:1000px] transition-all duration-500 ease-in-out flex-shrink-0",

        position === "discard" &&
          `absolute playing-card-${index < 3 ? index : 2}`
      )}
    >
      <div
        className={cn(
          ...
          {
            "ease-out duration-500 delay-0": position === "hand",
          },
      ...
      >
        <div className="front absolute top-0 left-0 [backface-visibility:hidden] w-full h-full">
          ...
          <div
            className={clsx(
              "absolute top-[63%] left-1/2 -translate-x-1/2 z-20 capitalize w-full text-center",
              {
            ...
            <h2
              className={clsx(
                ...
                "text-[1.34vw] w-full font-bold text-transparent bg-gradient-to-t bg-clip-text",
                {
                  ...
                  "from-[#424A4A] to-[#E97451]":
                    name?.includes("shoot") && name?.length > 5,
                  "from-[#1B5E4A] to-[#3FD4A7]": name === "pan",
                  "from-[#424A4A] to-[#3ED3A6]":
                    name?.includes("pan") && name?.length > 3,
                }
              )}
            >
          ...
          <div
            className={clsx(
              "absolute top-[79%] left-1/2 -translate-x-1/2 z-20 w-full text-center px-2",
              {
                "top-[84%]": name?.includes("shoot") && name.length > 5,
              }
            )}
          >
            <p
              className={clsx("text-[0.66vw] w-full font-medium text-white/80")}
            >
