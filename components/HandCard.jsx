...
export default function HandCard({
  ...
  const cardWidth = 9;
  const cardGap = 1;
  const cardSpace = hand.length * cardWidth + cardGap * (hand.length - 1);
  let checkFieldSpace = fieldWidth - (3 * cardWidth + cardGap * (hand.length - 1)) < fieldWidth / 2;

  let card1Right = checkFieldSpace
    ? (fieldWidth - cardSpace) / 2 + 9
    : (fieldWidth - cardSpace) / 2 ;
  if (player === 1)
    console.log("fieldWidth", fieldWidth, "cardSpace", cardSpace);
